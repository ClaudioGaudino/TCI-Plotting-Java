...
class NMFService(object):
    ...
    def getKOptimal(self):
        k = KMeansResult().res['k_optimal']
        return k

    def getAssignments(self):
        assignments = KMeansResult().res['assignments']
        assignments = [list(t) for t in assignments]

        return ListConverter().convert(assignments, gateway._gateway_client)

    def getMedianProfiles(self):
        profiles = KMeansResult().res['meadian_profiles']

        return ListConverter().convert(profiles, gateway._gateway_client)

    def getStdProfiles(self):
        profiles = KMeansResult().res['std_progiles']

        return ListConverter().convert(profiles, gateway._gateway_client)

    def getIntraSimilarity(self):
        simil = KMeansResult().res['intra_similarity']
        simil = [list(t) for t in simil]

        return ListConverter().convert(simil, gateway._gateway_client)

    def getInterSimilarity(self):
        simil = KMeansResult().res['inter_similarity']
